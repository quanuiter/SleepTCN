...
\newcommand{\ci}{\ensuremath{95\%\,\mathrm{CI}}}
...
replacement, and preprocessing. Paired subject-cluster bootstrap and Wilcoxon--Holm analysis assessed uncertainty.
...
families. These findings highlight the importance of evaluating preprocessing, stage-specific errors and
cross-cohort reliability when developing automated EEG-based sleep-staging systems.
\end{abstract}
...
For automated sleep-staging systems, such robustness is important because models trained on one cohort
may encounter different populations, acquisition systems and EEG derivations in subsequent use.
...
Studies published in medical-informatics settings have likewise examined automatic sleep scoring with
uni- and bidirectional LSTM models on single-channel Sleep-EDF EEG~\cite{fraiwan2020lstm}, an
interpretable knowledge-based system tested on heterogeneous sleep-disorder recordings~\cite{vanbuis2020userfriendly},
and a convolutional approach using raw EEG for sleep-stage classification~\cite{sundar2021cnn}.
These studies demonstrate the feasibility of automated sleep scoring across recurrent, knowledge-guided
and convolutional approaches, whereas the present work focuses on how pipeline and preprocessing choices
behave under a controlled cross-cohort evaluation.
...
To compare the specified model, training and preprocessing configurations, we evaluate the end-to-end
pipeline illustrated in Figure~\ref{fig:pipeline}. The signal is calibrated and processed at the
continuous-recording level before epoching to preserve amplitude relationships; a 1D-ResNet epoch
encoder and a TCN sequence model then extract local and temporal characteristics, respectively.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{./figures/gate8_pipeline_overview_en.png}
    \caption{\textbf{End-to-end processing pipeline for the evaluated ResNet-1D--TCN configuration
    (E3).} The raw continuous EEG signal is sequentially
...
memory. Figure~\ref{fig:speedup_tradeoff} summarizes the measured speed--performance trade-off for the
...
therefore faster but not smaller. In the forward benchmark, E1 reuses
...
A post-hoc seed-123 extension evaluated E4 (band-pass only) on the opened SHHS1 sample. E4
...
evidence that the additional fixed scaling improves transfer. These comparisons are post-hoc because
they were specified and conducted after the SHHS sample had been opened.
...
In the supplementary context ablation, the protocol-specified CPN--C contrast showed no detectable
marginal difference in boundary \mf\ ($\Delta=0.000953$; \ci\ $[-0.004588,0.006568]$; Holm
$p=1.000$). This result is limited to the explicit P/N feature groups within the evaluated E1 encoding
and does not establish that temporal context is unnecessary (Supplementary
Table~\ref{SM-tab:sup_context} and Fig.~\ref{SM-fig:sup_representation_context}).
...
These results support evaluating preprocessing and external-cohort behaviour as part of the development
of automated EEG analysis pipelines, rather than selecting models solely from in-domain benchmark
performance.
