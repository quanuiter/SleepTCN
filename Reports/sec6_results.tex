...
E3 & \textbf{0,790443} & 0,788265 \\
E4 & 0,789067 & \textbf{0,788673} \\
...
Cả bốn chênh lệch giữ hướng dương ở hai seed. E3--E6 là đối chiếu duy nhất có CI bootstrap hoàn toàn dương trong cả hai lần chạy; seed 42 đạt ý nghĩa Wilcoxon sau Holm, còn seed 123 không đạt. Hướng hiệu ứng được lặp lại, nhưng độ lớn giảm từ 0,0213 xuống 0,0102. E4 nhỉnh hơn E3 về Macro-F1 gộp ở seed 123. Kết quả E1--E0 và E2--E1 vẫn chưa đủ bằng chứng ở seed 123. Hai seed cố định chỉ cung cấp bằng chứng về độ nhạy, không phải một chiến dịch khẳng định độc lập bổ sung hoặc ước lượng phân phối biến thiên do khởi tạo.
...
Thời gian wall-clock còn phụ thuộc vào cấu hình optimizer, batch size, số epoch thực tế, quy tắc dừng sớm, điều phối lượt chạy, implementation và phần cứng; do đó các tỷ lệ trên là kết quả vận hành của chiến dịch hiện tại, không phải hiệu ứng kiến trúc riêng lẻ hay hằng số tốc độ phổ quát.
...
\subsection{Chiến dịch chuyển miền SHHS1 không cập nhật trọng số}
...
\begin{table}[H]
\centering
\caption{Hiệu năng chuyển miền trên 180 đối tượng SHHS1 đã khóa, không cập nhật trọng số}
...
\begin{table}[H]
\centering
\caption{Hai so sánh chuyển miền chính bắt cặp theo đối tượng}
...
Các cặp nhầm lẫn lớn nhất của E3 trên SHHS1 là N3$\rightarrow$N2 (16.674 epoch; nhãn tham chiếu N3, dự đoán N2), N2$\rightarrow$REM (11.605), N2$\rightarrow$N1 (5.947), W$\rightarrow$N1 (4.481) và N1$\rightarrow$REM (1.717). So với E2, số epoch N2 bị gán thành REM giảm 5.702, trong khi số epoch N3 bị gán thành N2 tăng 147. F1 N3 vẫn tăng nhẹ do số dự đoán đúng N3 tăng và sự cân bằng precision--recall thay đổi. Đây là mô tả trên mẫu đã khóa, không phải bằng chứng về cơ chế nhân quả hay một thống kê về chuyển pha giấc ngủ.

Đối chiếu với mốc 15-CNN--BiLSTM E0 cho thấy lỗi N3 không phát sinh riêng ở E3. Trên cùng 22.806 epoch N3, E0 gán 16.480 epoch thành N2 (72,3\%) và đạt recall N3 0,2610; E3 gán 16.674 epoch thành N2 (73,1\%) và đạt recall 0,2582. F1 N3 của hai mô hình gần như bằng nhau (0,4048 và 0,4055), mặc dù \mf\ gộp của E3 cao hơn E0. Bảng~\ref{tab:shhs_n3_crossmodel} vì vậy tách hai kết luận: E3 cải thiện hiệu năng tổng thể, nhưng thay đổi kiến trúc và pipeline không giải quyết được nhận diện N3 ngoài miền.

\begin{table}[H]
\centering
\caption{Đối chiếu lỗi N3 trên SHHS1 giữa các cấu hình đã khóa}
\label{tab:shhs_n3_crossmodel}
\begin{tabular}{lrrrrr}
\toprule
\textbf{Cấu hình} & \textbf{Precision N3} & \textbf{Recall N3} & \textbf{F1 N3} & \textbf{N3$\rightarrow$N2} & \textbf{Tỷ lệ} \\
\midrule
...
E6 kiểm tra pipeline dùng z-score theo bản ghi ở cả nguồn khi huấn luyện và đích khi suy luận. Đây không phải phép thêm chuẩn hóa lên cùng trọng số E3 đã đóng băng. Bảng~\ref{tab:shhs_e6_perclass} cho thấy pipeline này không khắc phục thiếu N3 trên mẫu đã đánh giá.
...
Tại vùng lân cận thay đổi nhãn, recall N3 của E6 là 0,0721 so với 0,0733 của E3; đây là các giá trị mô tả gần nhau, không phải kết luận tương đương. Cùng với recall toàn mẫu thấp hơn, kết quả cho thấy pipeline z-score đã thử không khắc phục thiếu N3. Đối chiếu không cô lập tác động của biên độ, không loại trừ các yếu tố liên quan biên độ/montage hoặc các phương pháp thích nghi không nhãn khác.

\subsubsection{Kiểm tra hậu nghiệm vùng lân cận thay đổi nhãn}

Phân tích hậu nghiệm dùng dự đoán test seed 42 đã khóa của E0 và E3. Với $j$ là epoch đầu của nhãn mới, vùng lân cận là hợp $\{j-1,j,j+1\}$. Vùng ổn định cách cả hai epoch $j-1$ và $j$ của mọi thay đổi nhãn ít nhất ba epoch, theo định nghĩa trong phương pháp. Hai vùng không bù nhau: Sleep-EDF có 43.237/130.693 epoch lân cận/ổn định và 21.539 epoch ngoài cả hai; SHHS1 tương ứng 44.744/103.195 và 21.073. Nhãn không hợp lệ ngắt chuỗi. Các vùng lấy từ nhãn tham chiếu, không từ dự đoán, nên đây là chẩn đoán ngoại tuyến chứ không phải bộ phát hiện chuyển pha có thể triển khai.

\begin{table}[H]
\centering
\caption{Hiệu năng tại vùng lân cận thay đổi nhãn và vùng ổn định (seed 42)}
\label{tab:transition_stable_crosscohort}
\resizebox{\textwidth}{!}{%
\begin{tabular}{llrrrr}
\toprule
\textbf{Cohort} & \textbf{Cấu hình} & \textbf{\mf\ lân cận} & \textbf{\mf\ ổn định} & \textbf{Recall N3 lân cận} & \textbf{Recall N3 ổn định} \\
\midrule
...
Cả hai mô hình có điểm thấp hơn ở vùng lân cận trên cả hai cohort, trong khi E3 vẫn cao hơn E0 về \mf\ tại vùng này. Kiểm tra chỉ giữ các thay đổi nhãn có ít nhất ba epoch liên tiếp cùng nhãn ở mỗi phía cho cùng hướng: \mf\ E0/E3 là 0,6253/0,6377 trên Sleep-EDF và 0,4591/0,4864 trên SHHS1. Tuy nhiên, recall N3 của E0/E3 ngay trong vùng ổn định SHHS1 vẫn chỉ là 0,4008/0,3900. Lỗi vì vậy không giới hạn ở vùng ranh giới. Thành phần lớp giữa các vùng khác nhau; so sánh này không xác định tác động nhân quả của chuyển pha sinh lý, thực hành chấm hay thiết bị.

\subsubsection{Đòn bẩy phản thực cho quyết định thích nghi}

\mf\ gộp của E3 là 0,7904 trên Sleep-EDF và 0,6099 trên SHHS1, chênh lệch 0,1805. Đây là khác biệt giữa EDF ngoài fold và SHHS ensemble mười mô hình, đồng thời khác phân bố lớp và thành phần cohort, không phải ước lượng riêng tác động dịch chuyển miền. F1 N3 giảm 0,4030, tương ứng khoảng 45\% chênh lệch Macro-F1 xét theo năm lớp. Trên SHHS1, số dự đoán N3 chỉ bằng 0,273 lần số nhãn N3 thật; 16.674/22.806 epoch N3 (73,1\%) bị gán thành N2. Precision cao--recall thấp mô tả việc phát N3 bảo thủ trên mẫu này, không phải bằng chứng calibration xác suất. Việc E0 biểu hiện gần cùng mức thiếu N3 cho thấy lỗi tái diễn qua hai họ mô hình được đánh giá, nhưng chưa xác định nguyên nhân là montage, tuổi, quy tắc chấm hay hình thái tín hiệu.
...
Đây không phải hiệu năng một thuật toán thực tế được bảo đảm đạt. Trong các kênh đã xét trên E3, N3$\rightarrow$N2 có mức tăng oracle lớn nhất, tiếp theo là N2$\rightarrow$REM. Tỷ phần trong bảng dùng chênh lệch điểm mô tả giữa hai điều kiện đánh giá làm mẫu số; sửa đồng thời có thể vượt điểm EDF ban đầu. Do Macro-F1 phi tuyến, không cộng các tỷ phần oracle riêng để dự đoán hiệu ứng đồng thời. Việc N3$\rightarrow$N2 cũng chiếm 72,3\% N3 ở E0 củng cố tính đáng chú ý của kênh lỗi, không biến oracle thành đối chiếu nhân quả hoặc bằng chứng rằng thích nghi có nhãn tối ưu hơn mọi hướng khác. Nếu kiểm chứng một biện pháp khắc phục, cần đánh giá đồng thời recall và false-positive N3 trên test không dùng để chọn biện pháp.
