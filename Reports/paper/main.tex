...
\begin{abstract}
Nghiên cứu đánh giá có kiểm soát các mô hình học sâu cho phân giai đoạn giấc ngủ từ EEG đơn kênh trên Sleep-EDF Expanded. Sáu cấu hình được so sánh trên cùng phép chia theo 78 đối tượng và cùng nguyên tắc lựa chọn mô hình. Cấu hình ResNet-1D--TCN với chế độ xử lý biên độ chính đạt \mf\ ngoài fold 0,7904, cao nhất trong chiến dịch. So với E6, trong đó z-score dùng trung bình và độ lệch chuẩn của toàn bộ bản ghi đích không nhãn, chênh lệch là 0,0213 (\ci\ $[0,0122;0,0307]$, $p_{Holm}=0,0012$). E6 vì vậy có tính transductive ở cấp bản ghi, không phải zero-shot thuần inductive. Lần lặp sau giao thức giữ hướng hiệu ứng dương. ResNet-1D--TCN đạt speed-up suy luận khoảng 3,76 lần, đổi lại số tham số cao hơn 4,37 lần và bộ nhớ GPU cực đại cao hơn 28,4\%. Trên 180 đối tượng SHHS1 đã khóa, E3 cao hơn hai mốc đối chứng theo \mf\ trung bình ở cấp đối tượng, nhưng E0 và E3 đều bỏ sót hơn 72\% epoch N3 thành N2 và có recall N3 gần như nhau (0,2610 và 0,2582). Kết quả ủng hộ lợi ích tổng thể và vận hành của quy trình E3, đồng thời định vị một failure mode N3 ngoài miền không được giải quyết bằng thay kiến trúc.
\end{abstract}

\noindent\textbf{Từ khóa:} phân giai đoạn giấc ngủ; EEG đơn kênh; TCN; dịch chuyển miền; thích nghi miền; kiểm định bắt cặp.
...
\begin{abstract}
Automatic sleep staging from single-channel electroencephalography requires protocols that separate model effects from subject and preprocessing differences. We compared six configurations on a subject-wise Sleep-EDF Expanded split. The fixed-scale ResNet-1D--TCN configuration E3 achieved the highest out-of-fold macro-F1 (0.7904) and exceeded E6 by 0.0213 (95\% CI $[0.0122,0.0307]$, Holm-adjusted $p=0.0012$). E6 estimates its mean and standard deviation from each complete unlabelled target recording and is therefore label-free but transductive. E3 provided a 3.76-fold inference speed-up, at the cost of 4.37 times more parameters and 28.4\% higher peak GPU memory. On 180 locked SHHS1 participants, E3 exceeded both references overall, but E0 and E3 each misclassified more than 72\% of N3 epochs as N2 and had similar N3 recall (0.2610 and 0.2582). The results support E3's overall and operational trade-off while identifying a cross-pipeline N3 transfer failure.
\end{abstract}
\noindent\textbf{Keywords:} sleep staging; single-channel EEG; temporal convolutional network; domain shift; target-domain adaptation; paired inference.
...
Nghiên cứu thiết kế một chiến dịch bắt cặp theo đối tượng để trả lời bốn câu hỏi: (1) TCN có cải thiện so với BiLSTM khi giữ nguyên bộ trích đặc trưng hay không; (2) việc thay gói đặc trưng/ngữ cảnh C/P/N 75 chiều của E1 bằng encoder ResNet-1D 128 chiều chỉ dùng epoch hiện tại có tạo lợi ích hay không; (3) chế độ xử lý biên độ ảnh hưởng thế nào; và (4) các kết quả có giữ được khi đánh giá chuyển miền sang SHHS1 hay không.

Đóng góp chính gồm: một giao thức 10-fold thống nhất và chống rò rỉ; các ablation thành phần trên cùng đối tượng; đánh giá độ phức tạp, tốc độ và không gian đặc trưng; phân tích trực tiếp vai trò của ngữ cảnh liền trước và liền sau; và một chiến dịch chuyển miền không cập nhật trọng số đã khóa trên SHHS1. Nghiên cứu phân biệt rõ so sánh định trước, phân tích thứ cấp và phân tích hậu nghiệm để giới hạn phát biểu trong phạm vi bằng chứng.
...
Tín hiệu được hiệu chuẩn về đơn vị vật lý và căn chỉnh với nhãn theo thời gian. Cửa sổ được xác định nhất quán giữa các cấu hình. Các điều kiện chính gồm tín hiệu thô, lọc dải, lọc dải kết hợp chế độ xử lý biên độ chính và lọc dải kết hợp z-score theo bản ghi. Với E6, trung bình và độ lệch chuẩn được tính trên toàn bộ bản ghi đích đã lọc và cắt biên mà không dùng nhãn; đây là chuẩn hóa bản ghi đích không nhãn có tính transductive, không phải suy luận zero-shot thuần inductive. Một biến thể xử lý được xác định trùng dữ liệu với điều kiện lọc dải nên không tạo thành một thí nghiệm độc lập.
...
E0 giữ cấu trúc CNN--BiLSTM của ZleepAnlystNet và được xem là đối chứng nội bộ đã hiệu chỉnh. E1 chỉ thay mô hình chuỗi bằng TCN và dùng lại encoder E0. E2 thay toàn bộ gói đặc trưng/ngữ cảnh C/P/N 75 chiều của E1 bằng encoder ResNet-1D 128 chiều chỉ nhận epoch hiện tại; vì vậy E2--E1 không phải phép cô lập riêng encoder. E3, E4 và E6 giữ ResNet-1D--TCN nhưng thay đổi chế độ xử lý tín hiệu. Các cấu hình dùng cùng nguyên tắc lựa chọn và đánh giá.
...
Trong ma trận nhầm lẫn E3, các cặp sai số lớn nhất là N3$\rightarrow$N2 (16.674), N2$\rightarrow$REM (11.605), N2$\rightarrow$N1 (5.947), W$\rightarrow$N1 (4.481) và N1$\rightarrow$REM (1.717). Đối chiếu cùng mẫu cho thấy E0 cũng gán 16.480/22.806 epoch N3 thành N2, tương ứng 72,3\%, và có recall N3 0,2610 so với 0,2582 của E3. Vì vậy, lỗi N3$\rightarrow$N2 là failure mode chuyển miền chung của hai họ kiến trúc; E3 cải thiện hiệu năng tổng thể nhưng không cải thiện nhận diện N3. Đây là các chẩn đoán mô tả trên mẫu SHHS1 đã khóa; chúng không được dùng để suy ra cơ chế nhân quả của kiến trúc hoặc tiền xử lý.

Trong kiểm tra hậu nghiệm dùng cùng định nghĩa vùng lân cận thay đổi nhãn, \mf\ của E0/E3 tại vùng lân cận so với vùng ổn định là 0,6179/0,6321 so với 0,8175/0,8346 trên Sleep-EDF, và 0,4323/0,4689 so với 0,5959/0,6220 trên SHHS1. Recall N3 tương ứng là 0,6121/0,6326 so với 0,9216/0,9316 trên Sleep-EDF, nhưng chỉ 0,0633/0,0733 so với 0,4008/0,3900 trên SHHS1. Kết quả tái diễn khi chỉ giữ các thay đổi nhãn có ít nhất ba epoch liên tiếp ở mỗi phía. Vì vùng ổn định SHHS1 vẫn suy giảm mạnh, thay đổi nhãn làm bài toán khó hơn nhưng không giải thích toàn bộ lỗi N3; đây là chẩn đoán hậu nghiệm từ nhãn tham chiếu, không phải kết luận về chuyển pha sinh lý hoặc bộ phát hiện trực tuyến.
...
Trong miền Sleep-EDF, E1--E0 và E2--E1 cho mức cải thiện mô tả dương nhưng chưa đạt ngưỡng sau Holm. Trên SHHS, E1--E0 cho hiệu ứng dương nhỏ, còn E2--E1 cho hiệu ứng âm. Chỉ E1--E0 là contrast kiến trúc thuần; E2--E1 thay cả encoder, P/N và số chiều đặc trưng. Mẫu hình này không ủng hộ tuyên bố ResNet-1D luôn tốt hơn bộ trích đặc trưng đối chứng. Giá trị thực dụng rõ nhất của ResNet-1D--TCN là kết hợp được hiệu năng cạnh tranh với cấu trúc vận hành gọn và tốc độ xử lý cao hơn, đổi lại nhiều tham số và bộ nhớ hơn.
...
Các kết quả nên được diễn giải trong phạm vi dữ liệu và giao thức đã đánh giá. SHHS1 đại diện cho một mẫu ngoài miền cụ thể, không phải toàn bộ quần thể. Lần lặp thứ hai củng cố hướng hiệu ứng nhưng chưa mô tả đầy đủ biến thiên do khởi tạo. Một số phân tích thành phần và phân tích vùng thay đổi nhãn được thực hiện sau khi mở mẫu nên chỉ có vai trò bổ trợ. EEG đơn kênh vẫn hạn chế khả năng phân biệt một số giai đoạn, đặc biệt N1 và REM; vùng lân cận được tạo từ nhãn tham chiếu ở hai phía nên chỉ phù hợp với đánh giá ngoại tuyến. Những giới hạn này không làm thay đổi kết quả chính, nhưng cần được tính đến khi mở rộng sang dữ liệu lâm sàng hoặc thiết bị khác.

\section{Kết luận}

Thiết kế thống nhất theo đối tượng và tập kiểm tra cho phép đánh giá công bằng sáu cấu hình trên Sleep-EDF. Ở chiến dịch chính, E3 đạt \mf\ mô tả 0,7904 và tốt hơn E6 với CI hoàn toàn dương cùng ý nghĩa sau Holm. Lần lặp sau giao thức giữ hiệu ứng E3--E6 theo hướng dương, củng cố bằng chứng về lợi ích của chế độ xử lý được lựa chọn. Phép thay TCN riêng và phép thay gói đặc trưng/ngữ cảnh E2 chưa tạo ưu thế ổn định sau hiệu chỉnh nhiều kiểm định. ResNet-1D--TCN đơn giản hóa vận hành và tăng tốc xử lý, với đánh đổi là nhiều tham số và bộ nhớ hơn. Trong chiến dịch seed 123, E2 hoàn tất huấn luyện và xác thực 10 fold trong 2 giờ 44 phút 57 giây, so với 33 giờ 35 phút 36 giây của E0; đây là lợi ích về thời gian thực đo trong giao thức cụ thể, không phải tuyên bố tốc độ độc lập phần cứng.
...
Phân tích này không nhận khoản tài trợ riêng nào từ cơ quan công, thương mại hoặc phi lợi nhuận.
Nghiên cứu Sleep Heart Health Study nền được hỗ trợ bởi các thỏa thuận hợp tác của National Heart,
Lung, and Blood Institute: U01HL53916, U01HL53931, U01HL53934, U01HL53937, U01HL64360, U01HL53938,
U01HL53940, U01HL53941 và U01HL63463.

\subsection*{Lời cảm ơn}

Nghiên cứu sử dụng Sleep-EDF Expanded do PhysioNet phân phối và dữ liệu từ Sleep Heart Health Study qua National Sleep Research Resource.

\subsection*{Đóng góp của tác giả (CRediT)}

Ngô Nhật Quân: Ý tưởng, phương pháp, phần mềm, kiểm định, phân tích hình thức, điều tra, quản lý dữ liệu,
trực quan hóa, viết bản thảo đầu tiên và hiệu đính. Phạm Thái Sơn: giám sát, phương pháp, kiểm định
và hiệu đính. Cả hai tác giả đã duyệt bản thảo cuối.
