...
\author[a,$\ast$]{Ngo Nhat Quan}
\author[a]{Pham Thai Son}
\affil[a]{Faculty of Information Systems, University of Information Technology,\protect\\
Vietnam National University Ho Chi Minh City,\protect\\
Quarter 34, Linh Xuan Ward, Ho Chi Minh City, Viet Nam}
...
partitions. Comparisons covered a temporal convolutional network (TCN) sequence-model and training-recipe
replacement, a bundled encoder/context replacement, and preprocessing. Paired bootstrap and
Wilcoxon--Holm analysis assessed uncertainty. Transfer used 180 locked Sleep Heart Health Study Visit 1
(SHHS1) participants without weight updates. Record-wise normalisation used each complete unlabelled target
recording and was transductive.

\noindent
\textbf{Results.} The fixed-scale one-dimensional residual network (ResNet-1D) with TCN
configuration (E3) reached Sleep-EDF \mf\ 0.7904
...
significance. E3 delivered a $3.76\times$ forward-pass speed-up over the convolutional--bidirectional
long short-term memory control,
...
over 72\% of N3 (deep-sleep) epochs as N2 (non-REM stage 2); N3 recall was 0.2610 for the control,
0.2582 for E3 and 0.2005 for E6.
...
magnitude of design choices can differ between in-domain evaluation and cross-cohort use.
...
To evaluate the specified architecture, context and preprocessing configurations, we
use an end-to-end processing pipeline, as illustrated in Figure~\ref{fig:pipeline}. The signal is
first calibrated and processed as a continuous recording to preserve amplitude relationships, after which a
1D-ResNet epoch encoder and a TCN sequence model extract local and temporal characteristics,
respectively.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{./figures/gate8_pipeline_overview_en.pdf}
    ...
    a six-block non-causal TCN with a 253-epoch receptive field before final Softmax classification. The
    schematic was drawn in TikZ with OpenAI Codex assistance; it depicts processing steps, not measured
...
All six configurations completed validation-based selection before the primary seed-42 test predictions
were evaluated. In
...
In the Sleep-EDF analysis, E3\,--\,E0 is labelled post-hoc throughout.
...
The speed--performance figure plots the reported seed-42 aggregate scores and benchmark speed-ups
using an editable PGFPlots source. Supplementary plots are generated from archived aggregate results
with the repository's plotting script; neither plotting workflow refits models or recomputes tests.
...
($[0.000305, 0.014520]$) but a non-significant Holm-adjusted Wilcoxon result ($p=0.8989$), with improvement in
...
memory. Figure~\ref{fig:speedup_tradeoff} summarises the measured speed--performance trade-off for the
locked primary seed-42 configurations E0, E3 and E6. On the locked SHHS1 sample, E3 combines the
measured forward-pass speed-up with higher aggregate performance than E0 and E6. The implementation is
therefore faster but not smaller. In the forward benchmark, E1 reuses the same encoder as E0 and changes
the sequence model, giving a $1.07\times$ speed-up. The larger E2--E0 speed-up accompanies the
replacement of 15 independently executed CNN branches with one ResNet-1D encoder in the implemented
pipeline.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{./figures/gate8_primary_speedup_f1_en.pdf}
    \caption{\textbf{Measured forward speed-up and aggregate \mf\ for the locked primary seed-42
    configurations.} Only E0, E3 and E6 are shown. The left panel reports pooled Sleep-EDF \mf; the
    right reports subject-mean \mf\ on the locked SHHS1 sample. Speed-up is relative to E0 in the fixed
    forward-pass benchmark. The plot was generated from archived aggregate results using an editable
    PGFPlots source; OpenAI Codex assisted with the plotting code.}
...
they provide complementary evidence on feature geometry and temporal context. In the Sleep-EDF E1
context ablation, adding the explicit Previous and Next (P/N) groups to the current-epoch features
produced no detectable boundary-performance gain ($\Delta\mf=0.0010$, \ci\ $[-0.0046,0.0066]$,
Holm $p=1.000$; 36/78 subjects improved). This result is conditional on the E1 encoding and specified
TCN recipe.
...
$\pm1$ epoch of a reference-label transition, E3's pooled boundary \mf\ exceeds E0 by $0.0366$ and E6 by $0.0183$.
...
post-hoc E3\,--\,E2 contrast is the largest of the configuration differences reported for this
SHHS1 extension ($+0.0475$, \ci\ $[0.0372,0.0579]$, $\dsign = +0.633$, 147/180 subjects improved).
E3 and E2 use the same model and specified training recipe with different preprocessing.
...
In this extension, band-pass-only preprocessing outperformed both the raw-input and fixed-scale
configurations under the same seed. These comparisons remain secondary
...
study remain appropriate next steps. We did not stratify performance by sex or gender, so subgroup-specific
robustness remains unassessed.
...
On SHHS1, the fixed-scale E3 procedure outperformed both locked references. Among the reported seed-42
configuration contrasts, the largest subject-mean difference was associated with preprocessing under
a fixed architecture and training recipe.
...
Sleep-EDF Expanded is available from PhysioNet
(\url{https://physionet.org/content/sleep-edfx/1.0.0/}); SHHS access and applicable use conditions are
managed by NSRR (\url{https://sleepdata.org/datasets/shhs}). Participant-level SHHS records and
predictions are not included in the manuscript package because access to the source records is controlled
by the data provider. The historical snapshot
...
An archived repository snapshot is available at Git commit
\href{https://github.com/quanuiter/SleepTCN/commit/f7c22e76022b8e24ac8d1343a39a990d42a1b3e2}{\texttt{f7c22e7}}.
The snapshot is cited for provenance of the retained source history; it is not asserted to be the
execution revision for every reported result.
...
University), U01HL53934 (University of Minnesota), U01HL53937 and U01HL64360 (Johns Hopkins
University), U01HL53938 (University of Arizona), U01HL53940 (University of Washington), U01HL53941
(Boston University), and U01HL63463 (Case Western Reserve University). The National Sleep Research
Resource was supported by the National Heart, Lung, and Blood Institute (R24 HL114473,
75N92019R002). The authors sincerely thank Nguy\~{\^e}n H\`{\^o} Duy Tr\'i for academic supervision, guidance in
defining the research topic, and constructive comments on earlier versions of the manuscript.
...
against code and archived results, reproducible plotting of existing aggregate data, and preparation of
the TikZ processing schematic. It did not generate new experimental observations, signals, predictions,
or numerical results, and it did not replace the archived predictions. No other AI tool was used for this
manuscript preparation. Both authors have read and approved the final manuscript and this disclosure; the
authors remain responsible for the submitted content.

%------------------------------------------------------------------------------
\begingroup
\small
\bibliographystyle{ieeetr-doi}
